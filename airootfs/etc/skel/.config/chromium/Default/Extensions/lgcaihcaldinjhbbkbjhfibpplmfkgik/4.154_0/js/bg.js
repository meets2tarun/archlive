...
function getNewsAddEditForm(info, tab) {
    chrome.permissions.request({
            ...
            if (granted) {
                newsObj = {tab: tab, u: info.pageUrl, t: tab.title, c: info.selectionText};
                chrome.tabs.executeScript(tab.id, {file: '/js/jquery.min.js'}, function() {
                    chrome.tabs.executeScript(tab.id, {file: '/js/jquery.Jcrop.min.js'}, function() {
                        chrome.tabs.executeScript(tab.id, {file: '/js/screenshot.js'}, function() {
                        });
                    });
                });
            }
            else {
                alert('Screenshot functionality has now been enabled for "News Posting". However, to enable Screenshot functionality, you have to provide permissions to Google Chrome.');
            };
        }
    );
