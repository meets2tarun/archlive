...
//    window.localStorage['password'] = JSON.stringify(this.password_);
    window.localStorage['jumboToken'] = this.token_;
    window.localStorage['password'] = this.password_;
  ...
  fetchToken_ : function() {
    ...
    if (window.localStorage['jumboToken'] !== undefined) {
      result = window.localStorage['jumboToken'];
      this.token_ = result;
    }
...
//    this.showProxyList();
  ...
  showProxyList: function() {
	...
	spanMissing.innerHTML = chrome.i18n.getMessage('manualProxiesMissing');
...
//					currentlySelectedIdentifier: 'english-proxy-one',
      ...
      for (var key in result) {
        if (result.hasOwnProperty(key)) {
          ...
          var status = result[key].status;
          var country = result[key].country;
          var city = result[key].city;
          var DMA = result[key].DMA;

	if( country == 'US') {
//		proxyList[0].servers.push({identifier:'us-proxy-' + id,description:country + ' proxy: ' + DMA});
//		proxyList[0].servers.push({identifier:'us-proxy-' + id,description:(country + ': ' + DMA).substr(0,19)});
		proxyList[0].servers.push({identifier:'us-proxy-' + id,description:country + ': ' + city});
//		proxyList[0].servers[id].identifier = 'us-proxy-' + id;
//		proxyList[0].servers[id].description = country + ' proxy: ' + DMA;
	} else if ( country == 'UK') {
//		proxyList[1].servers.push({identifier:'uk-proxy-' + id,description:(country + ': ' + DMA).substr(0,19)});
		proxyList[1].servers.push({identifier:'uk-proxy-' + id,description:country + ': ' + city});
...
/*					var marquee = document.createElement("marquee");
					...
					marquee.addEventListener("mouseover",function(){ this.scrollAmount = 3});
					marquee.onmouseout="this.scrollAmount = 0";
					marquee.width="70%";
					marquee.behavior="alternate";
					marquee.direction="right";
					marquee.appendChild(description);
					button.appendChild(marquee);
*/
