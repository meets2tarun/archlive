...
window.onload = function() {
	...
	(function() {
		...
		// get whether to use Secure Web Proxy
		function setSecureWebProxy(value) {
...
//			console.log('jumboToken: ' + window.localStorage['jumboToken']);
			return window.localStorage['jumboToken'];
		}
...
//    req.onerror = this.showError_.bind(this);
			...
			while (supportForm.lastChild) {
				supportForm.removeChild(supportForm.lastChild);
			}
			// Notification about message sent
			var success = document.createElement('h3');
			success.textContent = "Request sent";
			supportForm.appendChild(success);
			var success = document.createElement('div');
			success.textContent = chrome.i18n.getMessage('supportFormSubmission');
			supportForm.appendChild(success);
		}
...
*/
	  ...
	  var tabButtons = container.querySelectorAll('.tab-buttons > li > button');
	    ...
	    // set handling for secure web proxy button
	    var secureWebProxy = document.getElementById('secure-web-proxy');
	    secureWebProxy.onclick = function() {
			var  value = secureWebProxy.getAttribute('class') === 'active';
	    	if (setSecureWebProxy(!value)) {
				if (value) {
			    	secureWebProxy.removeAttribute('class');
			    }
			    else {
			    	secureWebProxy.setAttribute('class', 'active');
			    }
			}
	    };
	    // Chrome seems to filter the click from top elements
	    var secureElements = secureWebProxy.querySelectorAll('*');
	    for (var i = 0; i < secureElements.length; i++) {
	    	secureElements.onclick = secureWebProxy.onclick;
	    }
/*
...
*/
		...
		// get initial value for secure web proxy
		if (getSecureWebProxy()) {
			secureWebProxy.setAttribute('class', 'active');
		}
		// get initial value for unique token
		document.getElementById('jumboToken').setAttribute('value', getUniqueToken());
		// add form submit handling for support request
		document.getElementById('support-form').onsubmit = function() {
			submitSupportRequest({
				'token': document.getElementById('jumboToken').value,
