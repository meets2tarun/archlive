...
drawatmosfps = response['drawatmosfps'];if(drawatmosfps == null)drawatmosfps = '24';
...
try {chrome.runtime.sendMessage({name: 'sendautoplay'});} catch(e) {}
}
}
	...
	try {
		...
		if (elapsed > fpsInterval) {
			...
			if (window.location.href.match(/((http:\/\/(.*youtube\.com\/.*))|(https:\/\/(.*youtube\.com\/.*)))/i)){
				...
				// old flash
				if(yttest){
				if ($("movie_player").getPlayerState() == 1) {drawAtmos(youtubewindow, item);}
				}
			}
